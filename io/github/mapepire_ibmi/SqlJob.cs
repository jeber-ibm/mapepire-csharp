...
*/
    ...
    */
    ...
    public ConnectionResult? connect(DaemonServer db2Server)  {
        ...
        // Todo.. Gather options into JSON
        // Build JSON
        StringBuilder sb = new StringBuilder("{");
        sb.Append("\"id\":\""+SqlJob.getNewUniqueId()+"\""); 
        sb.Append(",\"type\":\"connect\"");
        sb.Append(",\"technique\":\"tcp\""); 
        sb.Append(",\"application\":\"C# client\"");
        sb.Append("}");
    String result; 
                    try {
                        result =  this.send(sb.ToString());
                    } catch (Exception ) {
    ...
    public Query Queryuery(String sql) {
